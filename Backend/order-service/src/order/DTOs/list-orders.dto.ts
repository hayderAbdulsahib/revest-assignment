  ...
  implements ValidatorConstraintInterface
{
  validate(createdTo: string, args: ValidationArguments) {
    const object = args.object as ListOrdersDto;
...
export class ListOrdersDto {
  ...
  @ApiPropertyOptional({
    ...
    default: OrderFields.CREATED_AT,
    enum: OrderFields,
  })
  @Transform(({ value }) => {
    if (value === undefined || value === null || value === '') {
      return OrderFields.CREATED_AT;
    }
    return value as string;
  })
  @IsOptional()
  @IsEnum(OrderFields, {
    message: `SortBy must be one of: ${Object.values(OrderFields).join(', ')}`,
  })
  sortBy?: OrderFields;
  ...
  @IsEnum(OrderStatusValues, {
    message: `Status must be one of: ${Object.values(OrderStatusValues).join(', ')}`,
  })
  ...
  @IsOptional()
  @IsString({ message: 'CustomerName must be a valid string' })
  customerName?: string;
  ...
  @IsOptional()
  @IsString({ message: 'CustomerEmail must be a valid string' })
  ...
  @IsOptional()
  @IsString({ message: 'CustomerPhone must be a valid string' })
  ...
  @IsOptional()
  @IsBoolean({ message: 'IsCanceled must be a valid boolean' })
  isCanceled?: boolean;
